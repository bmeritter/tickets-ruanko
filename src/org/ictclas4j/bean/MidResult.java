		...
		if (source != null) {
					...
					for (int j = 1; j <= width; j++) {
						...
						for (SegNode sn : segGraph) {
							if (i == sn.getRow() && j == sn.getCol()) {
								html.append("<td width=\"" + (95 / (double) width) + "%\"><a title=\"pos="
										+ sn.getPos() + " value=" + sn.getValue() + "\">" + sn.getWord() + "</a></td>");
								flag = true;
								break;
							}
						}
			...
			if (firstResult != null && firstResult.size() > 0) {
				...
				for (int i = 0; i < firstResult.size(); i++) {
					html.append("<tr>");
					html.append("<td width=\"10%\">" + i + "</td>");
					html.append("<td width=\"90%\" ><font color=\"#FF0000\"><b>" + firstResult.get(i)
							+ "</b></font</td>");
					html.append("</tr>");
				}
				html.append("</table>");
